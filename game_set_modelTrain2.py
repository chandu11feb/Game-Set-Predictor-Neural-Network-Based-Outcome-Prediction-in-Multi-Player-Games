    ...
    for epoch in range(epochs):  # Iterate over epochs
        ...
        for game_sequence, next_winner in zip(X_train, y_train):  # Iterate over game sequences and labels
            for i in range(1, len(game_sequence)):  # Iterate over sets with context
                # print(i)
                current_winner = torch.tensor(game_sequence[:i]).to(device)  # Extract the current sequence
                # one_hot_input = torch.nn.functional.one_hot(current_winner.long().to(device), num_classes=3)  # Convert to one-hot encoding
                outputs = model(current_winner.float())  # Forward pass through the model
                # print(outputs)
                loss = criterion(outputs.view(-1), torch.tensor(next_winner[i-1]).to(device) )  # Compute the loss by comparing with actual winner.
...
print(f'Before Loading data : {time.ctime()}')

# dataset_file = "newGamesDataset.txt"
